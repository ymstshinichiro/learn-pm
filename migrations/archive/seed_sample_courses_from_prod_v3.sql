...
-- Version 3: Simplified approach without ON CONFLICT on lessons

-- First, ensure pm-basic and pm-advanced courses exist with correct is_public setting
...
-- Insert lessons and questions using procedural code
DO $$
DECLARE
  pm_basic_id INTEGER;
...
BEGIN
  ...
  -- Delete and re-insert lessons for pm-basic (to ensure clean state)
  DELETE FROM lessons WHERE course_id = pm_basic_id;
...
- **プロジェクトの種類によって異なる**: 建設プロジェクトとソフトウェア開発プロジェクトでは、ライフサイクルが異なります', 2, '2025-10-22 05:15:43.8724');
  ...
  -- Delete and re-insert lessons for pm-advanced (to ensure clean state)
  DELETE FROM lessons WHERE course_id = pm_advanced_id;
...
- ステータス', 2, '2025-10-22 06:47:50.58125');
  ...
  -- Insert questions for what-is-project lesson
  ...
  -- Insert questions for project-lifecycle lesson
  INSERT INTO questions (lesson_id, type, question, options, correct_answer, explanation, "order", created_at)
  VALUES
  ...
  -- Insert questions for stakeholder-management lesson
  INSERT INTO questions (lesson_id, type, question, options, correct_answer, explanation, "order", created_at)
  VALUES
  ...
  -- Insert questions for risk-management lesson
  INSERT INTO questions (lesson_id, type, question, options, correct_answer, explanation, "order", created_at)
  VALUES
